...
void Store::saveDuration(const std::string &windowName, std::chrono::seconds duration) {
    ...
    try {
        ...
        SQLite::Statement query(db, "INSERT INTO durations (window, duration) VALUES (?, ?);");
        ...
        std::cerr << "Failed to save duration to the database: " << e.what() << std::endl;
    }
    // std::ofstream file(m_storePath, std::ios::app);
    // if (!file.is_open()) {
    //     std::cerr << "Failed to open store file while saving durations" << std::endl;
    //     m_missed_durations.insert(std::pair{windowName, duration.count()});
    //     return;
    // }
    //
