...
static void onActiveWindowChange(void *self, SCallbackInfo &info,
                                 std::any data) {
  try {
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
  ...
  if (HASH != GIT_COMMIT_HASH) {
    HyprlandAPI::addNotification(
        PHANDLE, "[HyprTracker] Mismatched headers! Can't proceed.",
        CColor{1.0, 0.2, 0.2, 1.0}, 5000);
    throw std::runtime_error("[HyprTracker] Version mismatch");
  }

  // Binding cerr to file
  std::ofstream err_file(std::format("{}/.local/share/hyprtracker/plugin.log",
                                     std::getenv("HOME")));
  if (!err_file.is_open()) {
  ...
  // TODO: Re-add the external server idea (ability to choose to where to save
  // to sqlite, webserver,...) HyprlandAPI::addConfigValue(PHANDLE,
  // "plugin:hyprtracker:endpoint", Hyprlang::STRING{"http://localhost:21435"});
  ...
  return {"HyprTracker", "Track the time spent in a window", "NuttyShrimp",
          "0.1"};
}
