...
void Viewer::create_date_selector() {
  ...
  m_state->signal_change().connect(
      sigc::mem_fun(*this, &Viewer::set_week_label));
  set_week_label();
  box.append(m_week_label);

  auto prev_btn = Gtk::Button();
  prev_btn.set_icon_name("go-previous");
  prev_btn.signal_clicked().connect([this] { m_state->move_a_week(-1); });
  box.append(prev_btn);

  auto today_btn = Gtk::Button();
  today_btn.set_label("This week");
  today_btn.signal_clicked().connect([this] { m_state->reset_time(); });
  box.append(today_btn);

  auto next_btn = Gtk::Button();
  next_btn.set_icon_name("go-next");
  next_btn.signal_clicked().connect([this] { m_state->move_a_week(1); });
  box.append(next_btn);
